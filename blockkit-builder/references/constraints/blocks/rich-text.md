...
### [Block:Rich Text]C30: Formatting consistency
Formatting style should be consistent throughout the block. Mixing conventions — such as switching between bold-for-emphasis and italic-for-emphasis in the same block — creates visual confusion.
**Confidence signals:** Multiple conflicting formatting patterns in one block = high. Minor inconsistency across long content = low.

### [Block:Rich Text]C31: Structure appropriate
Content should use the rich text sub-type that matches its nature: lists for enumerations, quotes for quotations, preformatted for code. Placing all content in plain `rich_text_section` paragraphs when structured sub-types would be clearer misses the purpose of rich text.
**Confidence signals:** Enumerated items in plain paragraphs with manual bullet characters = high. Prose content that could arguably be a list = low.
