...
- `description` — exact description of what this step does. This becomes the task subject verbatim — not a summary, not shortened, the exact text. Must be specific enough to execute without additional context.
...
- `acceptance_criteria` — array of individually checkable conditions. Each must be binary pass/fail. These are what the Auditor uses to verify correctness after execution. Not "it works" but "the file at `src/api/users.ts` contains `/api/v2/users` on line 45."
...
If the user has concerns, revise. Do not proceed on assumption — the user must explicitly approve. "Looks good", "go ahead", "yes" count as approval. Silence, "hmm", or "let me think" do not. If the user says anything is wrong, redo the affected steps and present again.
...
- The plan is a complete sequential recipe — no ambiguity, no "then figure out X"
...
- Iterate with the user until the user explicitly approves — no proceeding on assumption
