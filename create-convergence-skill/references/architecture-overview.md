...
An artifact (code, document, schema, config) gets iteratively improved by four independent agents converging from opposite directions — structural validity and contextual fitness — through a single guardian that merges their perspectives and dispatches fixes.

## The four roles

**Guardian** — owns the process start to finish. Dispatches the other agents, receives their reports, merges findings into a single fix report, dispatches the fixer, tracks state across rounds, decides when the loop is done or when to surface unresolved findings to the human. Sees everything: constraints + domain + context.
...
**Fixer** — the only agent that touches the artifact. Receives the guardian's fix report and makes surgical, minimal corrections. Sees the fix report and constraints only.
...
The sweeper and optimizer are intentionally isolated from each other:

- **Sweeper sees constraints only** — so it can't self-censor findings based on "well, in this domain that's actually fine." A constraint violation is a constraint violation. The guardian (which sees both lenses) resolves whether it should be actioned.
...
- **Neither sees the other's report** — preventing cross-contamination. Independent assessment from opposite directions produces richer signal for the guardian to merge.
...
The guardian uses these to decide:
...
2. The guardian is the only agent that can end the skill or surface residual
3. The optimizer runs minimum once per invocation (guarantees fitness assessment even if the sweeper finds nothing)
...
6. Only the guardian holds state across rounds — sweeper and optimizer are stateless
...
|------|---------|-----------|
| `constraints.md` | Structural rules (hard-reject + scored with confidence signals) | Sweeper, Guardian, Fixer |
| `domain.md` | Field knowledge, quality signals, norms | Optimizer, Guardian |
| `context.md` | Audience, venue, thresholds, pipeline position | Optimizer, Guardian |
| `gotchas.md` | Known pitfalls from previous runs | All agents |

The agent files (sweeper.md, guardian.md, fixer.md, optimizer.md) define roles and responsibilities and do NOT change between domains. Only the reference files change.
